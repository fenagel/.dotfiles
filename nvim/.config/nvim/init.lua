...
vim.keymap.set(
  "n",
  "<leader>tc",
  utils.toggle_set_color_column,
  { noremap = true, silent = true, desc = "[T]oggle [C]olorcolumn" }
)
    ...
    config = function()
      require("gitsigns").setup({
        on_attach = function(bufnr)
          ...
          map("n", "<leader>htb", gs.toggle_current_line_blame, { desc = "[T]oggle Blame Line" })
          map("n", "<leader>hd", gs.diffthis, { desc = "[D]iff This" })
          map("n", "<leader>hD", function()
            gs.diffthis("~")
          end, { desc = "[D]iff This ~" })
          map("n", "<leader>htd", gs.toggle_deleted, { desc = "[T]oggle Deleted" })
